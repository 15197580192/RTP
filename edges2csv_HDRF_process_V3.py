...
def getVex(inPath):
    with open(inPath+'vexKey_HDRF.txt', 'r', encoding='utf-8') as txtfile:
        for line in  txtfile:
            temp = line.replace('\n','').split(' ')
            temp1 = temp[0].split('@')
            csvVexKey[temp1[1]][temp[1]] = temp1[0]
    
    with open(inPath+'vexBelong_HDRF.txt', 'r', encoding='utf-8') as txtfile:
        for line in  txtfile:
            temp = line.replace('\n','').split(' ')
...
def getEdges(inPath, num):
    ...
    with open(inPath, 'r', encoding='utf-8') as txtfile:
        for i,line in enumerate(txtfile):
            ...
            # 移除换行符
            line = line.strip('\n')
            # 第一步：按": "分割（冒号+空格），拆分出 顶点对 和 分区编号
            parts = line.split(': ')
            if len(parts) != 2:
                print(f'第{i+1}行格式错误，跳过：{line}')
                continue
            # 第二步：按逗号分割顶点对，得到起点和终点
            vertex_pair = parts[0].split(',')
            if len(vertex_pair) != 2:
                print(f'第{i+1}行顶点格式错误，跳过：{line}')
                continue
            # 组装成和原代码一致的temp数组格式 [起点ID, 终点ID, 分区编号]
            temp = [vertex_pair[0], vertex_pair[1], parts[1]]
            # ========== 核心修改结束 ==========
            ...
            if((key1+'@'+key2) in csvEdges[temp[2]]):
                ...
                print(f'第{i+1}行边未找到对应分区：{line}')
                # quit()  # 建议替换为打印错误后继续，避免程序直接退出
                continue
