...
def getVex(inPath):
    with open(inPath+'vexKey_sheep.txt', 'r', encoding='utf-8') as txtfile:
        for line in  txtfile:
            temp = line.replace('\n','').split(' ')
            temp1 = temp[0].split('@')
            csvVexKey[temp1[1]][temp[1]] = temp1[0]
    
    with open(inPath+'vexBelong_sheep.txt', 'r', encoding='utf-8') as txtfile:
        for line in  txtfile:
            temp = line.replace('\n','').split(' ')
    ...
    for key in csvVexBelong:
        ...
        with open(inPath+key, 'r', encoding='utf-8') as oldcsvfile:
            ...
            with open(outPath+str(idx)+'/'+key, "w", encoding="utf-8", newline="") as newcsvfile:
                oldcsv_reader = csv.reader(oldcsvfile)
...
# nohup python3 -u edges2csv_sheep_process_V3.py >edges2csv_sheep.log 2>&1 &
if __name__ == "__main__": 
    start = time.perf_counter()
    inPath = '/data1/hzy/RTP/import/' # 原csv文件地址
    outPath = '/data1/hzy/neo4j/partition_code/result/result_sheep/30/csv/' # 新分区后的csv文件保存地址
    getVex('/data1/hzy/neo4j/partition_code/result/result_sheep/30/') # 获取点信息
    getEdges('/data1/hzy/neo4j/partition_code/result/result_sheep/30/result.txt', 8) # 获取8个分区的边信息
