...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/dbpedia-7-other.txt";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 3) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[7638400];
                    MPI_Recv(buffer, 7638400, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                for (int j = 0; j < query2Results.size(); j++) {
                    for (int k = 0; k < query1Results.size(); k++) {
                        if (query2Results[j].nodeId == query1Results[k].nodeId) {
