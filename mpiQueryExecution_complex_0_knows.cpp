    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 0) {
            ...
            if (world_rank == 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[347413];
                    MPI_Recv(buffer, 347413, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[30178165];
                    MPI_Recv(buffer, 30178165, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
            ...
            cout<<"knows2:"<<elapsed2.count()<<endl;
        }
        if (i == 2) {
            int pos = query_list[i].find('\n');
            string str1 = query_list[i].substr(0, pos + 1);
            ...
            for (int t = 0; t < epoch; t++) {
                ...
                if (world_rank != 0) {
                    ...
                    // vector<string> results_1(8);
                    for (int j = 1; j < world_size; ++j) {
                        char buffer[22893651];
                        MPI_Recv(buffer, 22893651, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                        results[j - 1] = string(buffer);
            ...
            // 计算所经过的时间
            std::chrono::duration<double> elapsed3 = end3 - start;
            cout<<"knows3:"<<elapsed3.count()<<endl;
