...
int getMonth(string tm) {
    ...
    pos = ttm.find('-');
    // cout << "tm: " << tm << " " << ttm.substr(0, pos) << endl;
...
int getDay(string tm) {
    ...
    // cout << "td: " << td << endl;
    // cout << "tm: " << tm << " " << td.substr(0, td.length() - 1) << endl;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank == 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[1631377];
                    MPI_Recv(buffer, 1631377, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                    auto now = chrono::system_clock::now();
                    auto duration_since_epoch = now.time_since_epoch();
                    auto seconds = chrono::duration_cast<chrono::seconds>(duration_since_epoch);
                ...
                for (int j = 0; j < person_list.size(); j++) {
                    ...
                    for (int k = 0; k < query1Results.size(); k++) {
                        if (person_list[j].id == query1Results[k].friendPerson.id) {
                            flag1 = true;
                            break;
                        }
                    }
        ...
        if (i == 4) {
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[1761361];
                    MPI_Recv(buffer, 1761361, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
