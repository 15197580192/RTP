...
int getDay(string tm) {
    ...
    // cout << "td: " << td << endl;
    return stoll(td.substr(0, td.length() - 1));
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-10-plus.txt";
    string neo4jResult;
    string tid = " ";
    string pid = " ";
    int tid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 0) {
            ...
            if (world_rank == 0) {
                ...
                for (int j = 0; j < query1_ans_list.size(); j++) {
                    ...
                    // cout<<"birthday"<<q1.friendPerson.birthday<<endl;
                    if (q1.friendPerson.birthday.substr(1,q1.friendPerson.birthday.length()-1)>"475760000000") {
        ...
        if (i == 3) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 0; j < query4_ans_list.size(); j++) {
                    ...
                    if (q4.friendPerson.id.length() < 1 || q4.friendPost.id.length() < 1) {
                        continue;
                    }
        ...
        // if (i == 4) {
        //     // cout<<"4query_list[i]"<<query_list[i]<<endl;
        //     int pos = query_list[i].find('\n');
        //     string str1 = query_list[i].substr(0, pos + 1);
        //     string str2 = query_list[i].substr(pos + 1);
                            ...
                            // for (int a = 0; a < query5Results.size(); a++) {
                                // if (query4Results[k].friendPost.id == query5Results[a].friendPost.id) {
                                    for (int b = 0; b < query3Results.size(); b++) {
                                        if(flag==1) break;
                                        if (query4Results[k].postTag.id == query3Results[b].likeTag.id) {
                                            commonPostCount++;
                                            flag = 1;
                                            break;
                                        }
