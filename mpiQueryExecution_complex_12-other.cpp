...
//     string file_path = "./queries/interactive-complex-12-other.txt";
//     string neo4jResult;
//     string tid = " ";
//     string tid1 = " ";
//     string tid2 = " ";
...
//                 for (int j = 1; j < world_size; ++j) {
//                     char buffer[2465318];
//                     MPI_Recv(buffer, 2465318, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//                     results[j - 1] = string(buffer);
...
//                 for (int j = 1; j < world_size; ++j) {
//                     char buffer[6065318];
//                     MPI_Recv(buffer, 6065318, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
//                     results[j - 1] = string(buffer);
...
//                         if (q5.postTag.id == query1Results[k].tag.id) {
...
//                                 if (query3Results[j].personComment.id == query4Results[k].personComment.id) {
//                                     for (int a = 0; a < query5Results.size(); a++) {
//                                         if (query4Results[k].commentPost.id == query5Results[a].commentPost.id) {
//                                             query2Results[i].tagNames.insert(query5Results[a].postTag.name);
//                                         }
//                                     }
//                                 }
//                             }
...
// vector<string> servers = {"bolt://10.157.197.82:26200/","bolt://10.157.197.82:26201/","bolt://10.157.197.82:26202/","bolt://10.157.197.82:26203/","bolt://10.157.197.82:26204/","bolt://10.157.197.82:26205/","bolt://10.157.197.82:26206/","bolt://10.157.197.82:26207/"};
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[244037361];
                    MPI_Recv(buffer, 244037361, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
        ...
        if (i == 3) {
            ...
            if (world_rank != 0) {
                ...
                // 输出结果
                std::cout << "Elapsed time1: " << elapsed2.count() << " seconds." << std::endl;
            }
            else {
                string new_query = str1 + tid1 + str2;
    ...
    // 输出结果
    std::cout << "Elapsed time2: " << elapsed2.count() << " seconds." << std::endl;
            }
            else {
                string new_query = str1 + tid2 + str2;
