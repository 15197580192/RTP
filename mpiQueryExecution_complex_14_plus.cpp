...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-14-plus.txt";
    string neo4jResult;
    string mid = " ";
    string nlist = " ";
    int mid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            string str1="WITH [";
            for(int a = 0; a < v_nlist.size()-1; a++){
                str1+='[';
                str1+=v_nlist[a];
                str1+=',';
                str1+=v_nlist[a+1];
                str1+=']';
                if(a<v_nlist.size()-2) str1+=',';
            }
            str1+="] AS idPairs\nUNWIND idPairs AS pair\nWITH pair[0] AS pA_id, pair[1] AS pB_id\n"+query_list[i];

            if (world_rank != 0) {
                string new_query = str1;
                ...
                for (int j = 0; j < query2_ans_list.size(); j++) {
                    Query2Result q6;
                    q6 = parseQuery2Results(query2_ans_list[j]);
                    query2Results.push_back(q6);
                }

                // cout << "cnt: " << cnt << endl;
                query1Results[0].pathWeight += query2Results.size();
            }
        ...
        if (i == 2) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 0; j < query3_ans_list.size(); j++) {
                    Query3Result q6;
                    q6 = parseQuery3Results(query3_ans_list[j]);
                    query3Results.push_back(q6);
                }

                // cout << "cnt: " << cnt << endl;
                query1Results[0].pathWeight += query3Results.size()*0.5;
            }
        ...
        //                         // cout<<"query5Results[j].m2Id: "<<query5Results[j].m2Id<<"query3Results[k].m2Id: "<<query3Results[k].m2Id<<endl;
        ...
        //                         // cout<<"query6Results[j].m2Id: "<<query6Results[j].m2Id<<"query4Results[k].m2Id: "<<query4Results[k].m2Id<<endl;
        ...
        //     }
        // }
        if (world_rank == 0&&i==query_list.size()-1) {
            auto now = chrono::system_clock::now();
            auto duration_since_epoch = now.time_since_epoch();
            auto seconds = chrono::duration_cast<chrono::seconds>(duration_since_epoch);
            auto milliseconds = chrono::duration_cast<chrono::milliseconds>(duration_since_epoch - seconds);
