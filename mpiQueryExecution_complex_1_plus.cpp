...
size_t findThirdSubstr(const string& mainStr, const string& subStr)
{
    // 边界判断：子串为空，直接返回查找失败
    if (subStr.empty())
        return string::npos;

    size_t subLen = subStr.length(); // 子串长度，用于下次查找的偏移
    size_t pos1, pos2, pos3;

    // 1. 找第一个子串的位置 (从下标0开始找)
    pos1 = mainStr.find(subStr);
    if (pos1 == string::npos) return string::npos;

    // 2. 找第二个子串的位置 (从第一个子串的结束位置开始找，避免重叠匹配)
    pos2 = mainStr.find(subStr, pos1 + subLen);
    if (pos2 == string::npos) return string::npos;

    // 3. 找第三个子串的位置 (从第二个子串的结束位置开始找)
    pos3 = mainStr.find(subStr, pos2 + subLen);
...
Query1Result parseQuery1Results(string data) {
    ...
    string split_flag=", 'companies'";
    int tpos = data.find(split_flag);
    stringSplit(data.substr(0,tpos), ", '", sub_data);
    for (int i = 0; i < sub_data.size(); i++) {
        // cout << sub_data[i] << endl;
        if (i == 0) {
            int pos = sub_data[i].find(':');
            int tnum = stoll(sub_data[i].substr(pos + 1));
            q1.distance = tnum;
        }
        if (i == 1) {
    ...
    string now_str = data.substr(tpos+split_flag.length(),data.length());
    // cout<<now_str<<endl;
    if (now_str.length() <= 2) {
        vector<string> t_v;
        t_v.push_back("null");
        v.push_back(t_v);
        q1.friendCompanies = v;
    }
    else{
...
Query3Result parseQuery3Results(string data) {
    ...
    for (int i = 0; i < sub_data.size(); i++) {
        ...
        if (i == 2) {
            int pos = sub_data[i].find(":");
            q3.friendCityName = sub_data[i].substr(pos + 2);
        }
    }
    return q3;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-1-plus.txt";
    string neo4jResult;
    string tid = " ";
    string oid = " ";
    string woid = " ";
        ...
        if (i == 0) {
            ...
            if (world_rank == 0) {
                ...
                for (int j = 0; j < query1_ans_list.size(); j++) {
                    Query5Result q1;
                    q1 = parseQuery5Results(query1_ans_list[j]);
                    query5Results.push_back(q1);
                    if (j == 0) {
                        tid = tid + q1.friendId;
                    }
                    else {
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 0; j < query5Results.size(); j++) {
                    ...
                    // t_v.push_back("null");
                    // v.push_back(t_v);
                    // q3.friendCompanies = v;
                    for (int k = 0; k < tq3.size(); k++) {
                        if (tq3[k].fid == q5.friendId) {
                            q5.friendUniversities = tq3[k].unis;
                            q5.friendCityName=tq3[k].friendCityName;
                            break;
                        }
                    }
                    if (q5.friendCompanies[0][0] == "null") {
                        ...
                        if (flag == 0) {
                            oid = oid + q5.friendCompanies[0][2];
                            flag = 1;
                        }
                        else {
                            oid = oid + ", " + q5.friendCompanies[0][2];
