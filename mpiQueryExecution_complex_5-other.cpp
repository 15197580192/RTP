...
bool cmp2(Query1Result a, Query1Result b) {
    return a.otherPerson.lastName < b.otherPerson.lastName;
}
...
// vector<string> servers = {"bolt://10.157.197.82:26200/","bolt://10.157.197.82:26201/","bolt://10.157.197.82:26202/","bolt://10.157.197.82:26203/","bolt://10.157.197.82:26204/","bolt://10.157.197.82:26205/","bolt://10.157.197.82:26206/","bolt://10.157.197.82:26207/"};
    ...
    sort(person_list.begin(), person_list.end(), cmp3);
    tid2 = "WHERE otherPerson.id IN [";
    for (int j = 0; j < person_list.size(); j++){
        // if (world_rank == 0) {
        //     cout << person_list[j].id << endl;
        // }
        if (j > 19) {
            break;
        }
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 2) {
            ...
            if (world_rank == 0) {
                ...
                sort(query1Results.begin(), query1Results.end(), cmp2);
                for (int j = 0; j < query1Results.size(); j++) {
                    if (j > 19) {
                        break;
                    }
                    if (j == 0) {
                        tid2 = tid2 + query1Results[j].otherPerson.id;
                    }
                    else {
                        tid2 = tid2 + ", " + query1Results[j].otherPerson.id;
                    }
        ...
        if (i == 3) {
            ...
            if (world_rank != 0) {
                ...
                // cout << world_rank << " new_query: " << new_query << endl;
                // while(1) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[4800000];
                    MPI_Recv(buffer, 4800000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
