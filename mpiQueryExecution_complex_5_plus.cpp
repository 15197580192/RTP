...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-5-plus.txt";
    string neo4jResult;
    string tid = " ";
    string fid = " ";
    string pid = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 2) {
            ...
            MPI_Bcast(&tid[0], tid_length, MPI_CHAR, 0, MPI_COMM_WORLD);
            int ppos=tid.find(" IN");
            tid="AND otherPerson2.id"+tid.substr(ppos,tid.length());

            if (world_rank != 0) {
                string new_query = str1 + fid + tid + str2;
                ...
                // vector<string> results_1(8);
                for (int j = 1; j < world_size; ++j) {
                    char buffer[116766958];
                    MPI_Recv(buffer, 116766958, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                for (int i = 0; i < query4Results.size(); i++) {
                    ...
                    for (int j = 0; j < query4Results.size(); j++) {
                        if (query4Results[i].friendPost.id == query4Results[j].friendPost.id) {
                            query4Results[i].postCount++;
                            break;
                        }
                    }
                }

                string now_id = query4Results[0].friendForum.id;
                int now_cnt = query4Results[0].postCount;
                for (int i = 1; i < query4Results.size(); i++) {
                    if (query4Results[i].friendForum.id == now_id) {
                        now_cnt += query4Results[i].postCount;
                        query4Results[i].postCount = -1;
                    }
                    else {
                        query4Results[i - 1].postCount = now_cnt;
                        now_id = query4Results[i].friendForum.id;
                        now_cnt = query4Results[i].postCount;
                    }
                }
