...
Query4Result parseQuery4Results(string data) {
    ...
    stringSplit(data, ", '", sub_data); // 如果comment里面末尾有逗号会有问题
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-8-plus.txt";
    string neo4jResult;
    string cid = " ";
    string ccid = " ";
    string pid = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        //         for (int j = 1; j < world_size; ++j) {
        //             char buffer[39745];
        //             MPI_Recv(buffer, 39745, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        //             results[j - 1] = string(buffer);
        ...
        if (i == 0) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[573203];
                    MPI_Recv(buffer, 573203, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                for (int j = 0; j < query5_ans_list.size(); j++) {
                    ...
                    // cout << "tt1: " << q4.replyComment.creationDate << endl;
