...
//         }
//         if (i == 1) {
//             q2.messageCreationDate = tdata;
//         }
//         if (i == 2) {
//             q2.messageContent = tdata;
//         }
//         if (i == 3) {
//             q2.postId = tdata;
//         }
...
Query2Result parseQuery2Results(string data) {
    ...
    // 提取 messageId
    size_t id_start = data.find("'messageId': '");
    if (id_start != string::npos) {
        id_start += 14;  // "'messageId': '" 的长度
        size_t id_end = data.find("'", id_start);
        if (id_end != string::npos) {
            q2.messageId = data.substr(id_start, id_end - id_start);
        }
    }
    
    // 提取 messageCreationDate
    size_t date_start = data.find("'messageCreationDate': '");
    if (date_start != string::npos) {
        date_start += 24;  // "'messageCreationDate': '" 的长度
        size_t date_end = data.find("'", date_start);
        if (date_end != string::npos) {
            q2.messageCreationDate = data.substr(date_start, date_end - date_start);
        }
    }
    
    // 提取 messageContent
    size_t content_start = data.find("'messageContent': '");
    if (content_start != string::npos) {
        content_start += 19;  // "'messageContent': '" 的长度
        size_t content_end = data.find("'", content_start);
        if (content_end != string::npos) {
            q2.messageContent = data.substr(content_start, content_end - content_start);
        }
    }
    
    // 提取 postId
    size_t postid_start = data.find("'postId': '");
    if (postid_start != string::npos) {
        postid_start += 11;  // "'postId': '" 的长度
        size_t postid_end = data.find("'", postid_start);
        if (postid_end != string::npos) {
            q2.postId = data.substr(postid_start, postid_end - postid_start);
        }
    }
    
    // 提取 personId
    postid_start = data.find("'personId': '");
    if (postid_start != string::npos) {
        postid_start += 13;  // "'personId': '" 的长度
        size_t postid_end = data.find("'", postid_start);
        if (postid_end != string::npos) {
            q2.personId = data.substr(postid_start, postid_end - postid_start);
        }
    }
    
    // 提取 personFirstName
    postid_start = data.find("'personFirstName': '");
    if (postid_start != string::npos) {
        postid_start += 20;  // "'personFirstName': '" 的长度
        size_t postid_end = data.find("'", postid_start);
        if (postid_end != string::npos) {
            q2.personFirstName = data.substr(postid_start, postid_end - postid_start);
        }
    }
    
    // 提取 personLastName
    postid_start = data.find("'personLastName': '");
    if (postid_start != string::npos) {
        postid_start += 19;  // "'personFirstName': '" 的长度
        size_t postid_end = data.find("'", postid_start);
        if (postid_end != string::npos) {
            q2.personLastName = data.substr(postid_start, postid_end - postid_start);
        }
    }
...
Query3Result parseQuery3Results(string data) {
    ...
    for (int i = 0; i < sub_data.size(); i++) {
        ...
        if (i == 2) {
            q3.personFirstName = tdata;
        }
        if (i == 3) {
            q3.personLastName = tdata;
        }
    }
    return q3;
}
...
bool cmp(Query2Result a, Query2Result b) {
    ...
    if (a.messageId.length() != b.messageId.length()) {
        return a.messageId.length() < b.messageId.length();
    }
...
void print(Query2Result q) {
    cout << "---------------------------------------" << endl;
    cout << "'messageId': " << q.messageId << ", 'messageCreationDate': " << q.messageCreationDate << ", 'messageContent': " << q.messageContent;
    cout << ", 'postId': " << q.postId << ", 'personId': " << q.personId << ", 'personFirstName': " << q.personFirstName;
    cout << ", 'personLastName': " << q.personLastName << endl;
    cout << q.icd << endl;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-short-2-plus.txt";
    string neo4jResult;
    string cid = " ";
    string pid = " ";
    int cid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        // cout << "当前节点: " << world_rank << endl; // todo
        ...
        if (i == 0) {
            if (world_rank == 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[890830];
                    MPI_Recv(buffer, 890830, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                ...
                for (int j = 0; j < query1_ans_list.size(); j++) {
                    ...
                    // if (j == 0) {
                    //     cid = cid + q1.messageId;
                    // }
                    // else {
                    //     cid = cid + ", " + q1.messageId;
                    // }
                    Query2Result q1;
                    q1 = parseQuery2Results(query1_ans_list[j]);

                    string ttime = q1.messageCreationDate;
                    // cout << j <<"ttime: " << ttime << endl;
                    int64_t timestamp = stoll(ttime);
                    q1.icd = timestamp;
                ...
                cout << "result_nums: " << query2Results.size() << endl; 
            ...
            cid_length = cid.size();
            // cout << "节点0的cid_length: " << cid_length << endl; // todo
        ...
        //             Query2Result q2;
        //             // std::cout<<j<<":"<<query2_ans_list[j]<<std::endl;
        //             q2 = parseQuery2Results(query2_ans_list[j]);
                    

        //             string ttime = q2.messageCreationDate.substr(1);
        ...
        //                 if(q3.postId == query2Results[k].postId) {
        //                     query2Results[k].personId = q3.personId;
        //                     query2Results[k].personFirstName = q3.personFirstName;
        //                     query2Results[k].personLastName = q3.personLastName;
        //                 }
        //             }
