...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-short-7-other.txt";
    string neo4jResult;
    string cid = " ";
    string cid1 = " ";
    string tid = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank == 0) {
                ...
                for (int j = 0; j < query1Results.size(); j++) {
                    if (j == 0) {
                        cid1 = cid1 + query1Results[j].commentId;
                    }
                    else {
                        cid1 = cid1 + ", " + query1Results[j].commentId;
                    }
        ...
        if (i == 4) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[1600000];
                    MPI_Recv(buffer, 1600000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                // cout << "query4Other_ans_list: " << query4Other_ans_list.size() << endl;

                tid1 = "WHERE a.id IN [";
                for (int j = 0; j < query4Other_ans_list.size(); j++) {
                    Query4Result q4;
                    q4 = parseQuery4Results(query4Other_ans_list[j]);
                    bool flag = false;
                    for(int k = 0; k < query4Results.size(); k++) {
                        if (query4Results[k].knowsPersonId == q4.knowsPersonId) {
                            flag = true;
                            break;
                        }
                    }
                ...
                for (int j = 0; j < query4Results.size(); j++) {
                    if (j == 0) {
                        tid1 = tid1 + query4Results[j].knowsPersonId;
                    }
                    else {
                        tid1 = tid1 + ", " + query4Results[j].knowsPersonId;
                    }
        ...
        if (i == 5) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[2600000];
                    MPI_Recv(buffer, 2600000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
