...
Query1Result parseQuery1Results(string data) {
    ...
    for (int i = 0; i < sub_data.size(); i++) {
        ...
        if (i == 3) {
            q1.replyAuthorId = tdata;
        }
        if (i == 4) {
            q1.replyAuthorFirstName = tdata;
        }
        if (i == 5) {
            q1.replyAuthorLastName = tdata;
        }
        if (i == 6) {
            q1.originalAuthorId = tdata;
        }
    }
    return q1;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-short-7-plus.txt";
    string neo4jResult;
    string cid = " ";
    string tid = " ";
    int cid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[1651313];
                    MPI_Recv(buffer, 1651313, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
