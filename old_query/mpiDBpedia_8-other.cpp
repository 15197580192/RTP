...
Node parseNode(string data) {
    ...
    while (getline(dataStream, token, ',')) {
        ...
        if (getline(getline(tokenStream, key, '='), value)) {
            ...
            else if (key == "key1") info.key1 = value;
            else if (key == "key2") info.key2 = value;
            else if (key == "key3") info.key3 = value;
        }
    }
...
//     cout << "---------------------------------------" << endl;
//     cout << "'nodeId': " << q.nodeData.id << "'key1': " << q.nodeData.key1 << "'key2': " << q.nodeData.key2 << "'key3': " << q.nodeData.key3 << endl;
// }
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/dbpedia-8-other.txt";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[3638400];
                    MPI_Recv(buffer, 3638400, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
        ...
        if (i == 2) {
            ...
            if (world_rank != 0) {
                ...
                // cout << query3_ans_list[0] << endl;

                for (int j = 0; j < query3_ans_list.size(); j++) {
                    Query3Result q3;
                    q3 = parseQuery3Results(query3_ans_list[j]);
                    // cout << "q3.id: " << q3.friendCity.id << endl;
                    query3Results.push_back(q3);
                }

                tid1 = "WHERE n1.id IN [" + query3Results[0].nodeId + "]\n";
            }
        ...
        if (i == 3) {
            while(true) {
                ...
                for (int t = 0; t < epoch; t++) {
                    query3_ans_list.clear();
                    string now_tid = "WHERE n1.id IN [";
                    for (int a = t * 200; a < min((t + 1) * 200, int(tlist.size())); a++) {
                        if (a % 200 == 0) {
                    ...
                    if (world_rank != 0) {
                        ...
                        // cout << "query3_ans_list: " << query3_ans_list.size() << endl;
                        // cout << query3_ans_list[0] << endl;
                        // cout << query1Results[9].nodeId << endl;
