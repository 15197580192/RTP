...
Query2Result parseQuery2Results(string data) {
    ...
    string td = data.substr(pos + 2);
    // cout << "td: " << td << endl;
    q2.shortestPathLength = td;
    return q2;
}

void print(Query2Result q) {
    cout << "---------------------------------------" << endl;
    cout << "'shortestPathLength': " << q.shortestPathLength << endl;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/dbpedia-8.txt";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            int pos = query_list[i].find("RETURN");
            string str1 = query_list[i].substr(0, pos);
            string str2 = query_list[i].substr(pos);
