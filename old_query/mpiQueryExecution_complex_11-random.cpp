...
Query2Result parseQuery2Results(string data) {
    Query2Result q2;
    // cout << "data:" << data << endl;
    Organisation o = parseOrganisation(data);
    q2.company = o;
    return q2;
}
...
Query3Result parseQuery3Results(string data) {
    ...
    for (int i = 0; i < sub_data.size(); i++) {
        ...
        if (i == 3) {
            q3.organizationName = tdata;
        }
        if (i == 4) {
            string ty = tdata.substr(1);
            ty = ty.substr(0, ty.length() - 1);
            q3.organizationWorkFromYear = stoi(ty);
        }
        if (i == 5) {
            q3.organizationId = tdata;
        }
    }
    return q3;
}
...
bool cmp(Query3Result a, Query3Result b) {
    if (a.organizationWorkFromYear != b.organizationWorkFromYear) {
        return a.organizationWorkFromYear < b.organizationWorkFromYear;
    }
    ...
    if (a.organizationName.length() != b.organizationName.length()) {
        return a.organizationName.length() > b.organizationName.length();
    }
...
void print(Query3Result q) {
    cout << "---------------------------------------" << endl;
    cout << "'personId': " << q.personId << ", 'personFirstName': " << q.personFirstName << ", 'personLastName': " << q.personLastName;
    cout << ", 'organizationName': " << q.organizationName << ", 'organizationWorkFromYear': " << q.organizationWorkFromYear << endl;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-11-random.txt";
    string neo4jResult;
    string tid = " ";
    int tid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[7465318];
                    MPI_Recv(buffer, 7465318, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                for (int j = 0; j < query3_ans_list.size(); j++) {
                    ...
                    if (q3.organizationWorkFromYear < 2011) {
                        for (int k = 0; k < query2Results.size(); k++) {
                            if (q3.organizationId == query2Results[k].company.id) {
