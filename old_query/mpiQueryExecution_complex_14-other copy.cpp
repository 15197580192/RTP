...
int main(int argc, char** argv) {
    ...
    vector<Query1Result> query1OtherResults;
    ...
    for (int i = 0; i < 3; i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank == 0) {
                ...
                // cout << "query1_ans_list: " << query1_ans_list.size() << endl;
                tid1 = "WHERE end.id IN [";
                
                for (int j = 0; j < query1_ans_list.size(); j++) {
                    Query1Result q1;
                    q1 = parseQuery1Results(query1_ans_list[j]);
                    query1OtherResults.push_back(q1);
                    if (j == 0) {
                        tid1 = tid1 + q1.friendId;
                    }
                    else {
                        tid1 = tid1 + ", " + q1.friendId;
                    }
        ...
        if (i == 2) {
            ...
            string flag = "false";
            while(1) {
                MPI_Bcast(&flag_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
                // cout << "flag_length: " << flag_length << endl;
                if (world_rank != 0) {
                    // cout << "这不是0号节点" << endl;
                    // 调整字符串大小以适应接收的数据
                    flag.resize(flag_length);
                }
                MPI_Bcast(&flag[0], flag_length, MPI_CHAR, 0, MPI_COMM_WORLD);
                ...
                int pos = query_list[i + 1].find('\n'); 
                string str1 = query_list[i + 1].substr(0, pos + 1);
                string str2 = query_list[i + 1].substr(pos + 1);
                ...
                if (world_rank == 0) {
                    for (int j = 0; j < query1Results.size(); j++) {
                        for (int k = 0; k < query1OtherResults.size(); k++) {
                            if (query1Results[j].friendId == query1OtherResults[k].friendId) {
                                flag = true;
                                for (int a = 0; a < query1Results.size(); a++) {
                                    if (query1Results[a].endId == query1Results[j].friendId) {
                                        nlist = query1Results[a].startId + ", " + nlist;
                                    }
                                }
                                for (int a = 0; a < query1OtherResults.size(); a++) {
                                    if (query1OtherResults[a].startId == query1OtherResults[k].friendId) {
                                        nlist = nlist + ", " + query1OtherResults[a].endId;
                                    }
                                }
                                break;
                            }
                ...
                flag_length = flag.size();
                // cout << "节点0的flag_length: " << flag_length << endl;
                MPI_Bcast(&flag_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
                flag.resize(flag_length);
                MPI_Bcast(&flag[0], flag_length, MPI_CHAR, 0, MPI_COMM_WORLD);
        ...
        for (int T = 0; T < 4; T++) {
            ...
            query5Results.clear();
            query6Results.clear();
            for (int t = 2; t < query_list.size(); t++) {
                if (t == 2) {
                    ...
                    if (world_rank != 0) {
                        ...
                        for (int j = 1; j < world_size; ++j) {
                            char buffer[160000];
                            MPI_Recv(buffer, 160000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            results[j - 1] = string(buffer);
                ...
                if (t == 3) {
                    ...
                    if (world_rank != 0) {
                        ...
                        for (int j = 1; j < world_size; ++j) {
                            char buffer[262144];
                            MPI_Recv(buffer, 262144, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                            results[j - 1] = string(buffer);
