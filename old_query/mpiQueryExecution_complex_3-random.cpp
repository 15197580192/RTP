...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-3-random.txt";
    string neo4jResult;
    string tid = " ";
    string tid1 = " ";
    string cities_id = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 2) {
            ...
            if (world_rank != 0) {
                string new_query = str1 + "WHERE city.id IN [" + cities_id + "]\n" + str2;
                // cout << "new_query" << new_query << endl;
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[5400000];
                    MPI_Recv(buffer, 5400000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
