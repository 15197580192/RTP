...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-4-random.txt";
    string neo4jResult;
    string tid = " ";
    string pid = " ";
    int tid_length = 0;
        ...
        if (i == 0) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[5242880];
                    MPI_Recv(buffer, 5242880, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            ...
            if (world_rank == 0) {
                ...
                for (int j = 0; j < query1_ans_list.size(); j++) {
                    ...
                    for (int k = 0; k < query2Results.size(); k++) {
                        if (q1.friendPerson.id == query2Results[k].friendId) {
                            if (cnt == 0) {
                                pid = pid + query2Results[k].friendPost.id;
                                cnt++;
                            }
                            else {
                                pid = pid + ", " + query2Results[k].friendPost.id;
