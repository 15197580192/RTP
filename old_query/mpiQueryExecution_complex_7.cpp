...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-7.txt";
    string neo4jResult;
    string cid = " ";
    string pid = " ";
    int cid_length = 0;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 3) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[540000];
                    MPI_Recv(buffer, 540000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
        ...
        if (i == 4) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[180000];
                    MPI_Recv(buffer, 180000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
