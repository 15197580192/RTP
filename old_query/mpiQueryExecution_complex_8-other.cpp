...
vector<string> query4Other_ans_list;
...
bool cmp(Query4Result a, Query4Result b) {
    if (a.commentCreationDate != b.commentCreationDate) {
        return a.commentCreationDate > b.commentCreationDate;
    }
    if (a.replyComment.id.length() != b.replyComment.id.length()) {
        return a.replyComment.id.length() < b.replyComment.id.length();
    }
    return a.replyComment.id < b.replyComment.id;
}

void print(Query4Result q4) {
    cout << "---------------------------------------" << endl;
    cout << "'personId': " << q4.commentPerson.id << ", 'personFirstName': " << q4.commentPerson.firstName << ", 'personLastName': " << q4.commentPerson.lastName;
    cout << ", 'commentCreationDate': " << q4.replyComment.creationDate << ", 'commentId': " << q4.replyComment.id;
    cout << ", 'commentContent': " << q4.replyComment.content << endl;
}
...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-8-other.txt";
    string neo4jResult;
    string cid = " ";
    string cid1 = " ";
    string ccid = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 2) {
            ...
            cid1_length = cid1.size();
            // cout << "节点0的cid1_length: " << cid1_length << endl;
            MPI_Bcast(&cid1_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            cid1.resize(cid1_length);
            MPI_Bcast(&cid1[0], cid1_length, MPI_CHAR, 0, MPI_COMM_WORLD);
        ...
        if (i == 3) {
            ...
            MPI_Bcast(&cid1_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            // cout << "cid1_length: " << cid1_length << endl;
            if (world_rank != 0) {
                // cout << "这不是0号节点" << endl;
                // 调整字符串大小以适应接收的数据
                cid1.resize(cid1_length);
            ...
            if (world_rank != 0) {
                string new_query = str1 + cid1 + str2;
                ...
                for (int j = 0; j < results.size(); ++j) {
                    ...
                    if (results[j].length() == 2) {
                        ...
                        vector<string> now_ans = getAnsList(results[j]);
                        for (int k = 0; k < now_ans.size(); k++) {
                            query3Other_ans_list.push_back(now_ans[k]);
                        }
                    }
                }

                // cout << "query3Other_ans_list: " << query3Other_ans_list.size() << endl;

                for (int j = 0; j < query3Other_ans_list.size(); j++) {
                    Query3Result q3;
                    q3 = parseQuery3Results(query3Other_ans_list[j]);
                    bool flag = false;
                    for (int k = 0; k < query3Results.size(); k++) {
                        if (query3Results[k].replyComment.id == q3.replyComment.id) {
                            flag = true;
                            break;
                        }
                    }
        ...
        if (i == 4) {
            ...
            if (world_rank != 0) {
                ...
                pid1 = "WHERE message.id IN [";

                for (int j = 0; j < query4_ans_list.size(); j++) {
                    Query3Result q3;
                    q3 = parseQuery3Results(query4_ans_list[j]);
                    query3Results.push_back(q3);
                    if (j == 0) {
                        pid1 = pid1 + q3.replyComment.id;
                    }
                    else {
                        pid1 = pid1 + ", " + q3.replyComment.id;
                    }
            ...
            pid1_length = pid1.size();
            // cout << "节点0的pid1_length: " << pid1_length << endl;
            MPI_Bcast(&pid1_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            pid1.resize(pid1_length);
            MPI_Bcast(&pid1[0], pid1_length, MPI_CHAR, 0, MPI_COMM_WORLD);
        ...
        if (i == 5) {
            ...
            MPI_Bcast(&pid1_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            // cout << "pid1_length: " << pid1_length << endl;
            if (world_rank != 0) {
                // cout << "这不是0号节点" << endl;
                // 调整字符串大小以适应接收的数据
                pid1.resize(pid1_length);
            ...
            if (world_rank != 0) {
                string new_query = str1 + pid1 + str2;
                ...
                for (int j = 0; j < results.size(); ++j) {
                    ...
                    if (results[j].length() == 2) {
                        ...
                        vector<string> now_ans = getAnsList(results[j]);
                        for (int k = 0; k < now_ans.size(); k++) {
                            query4Other_ans_list.push_back(now_ans[k]);
                        }
                    }
                }
                ...
                for (int j = 0; j < query4Other_ans_list.size(); j++) {
                    Query3Result q3;
                    q3 = parseQuery3Results(query4Other_ans_list[j]);
                    bool flag = false;
                    for (int k = 0; k < query3Results.size(); k++) {
                        if (query3Results[k].replyComment.id == q3.replyComment.id) {
                            flag = true;
                ...
                for (int j = 0; j < query3Results.size(); j++) {
                    if (j == 0) {
                        ccid = ccid + query3Results[j].replyComment.id;
                    }
                    else {
                        ccid = ccid + ", " + query3Results[j].replyComment.id;
                    }
            ...
            ccid_length = ccid.size();
            // cout << "节点0的ccid_length: " << ccid_length << endl;
            MPI_Bcast(&ccid_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            ccid.resize(ccid_length);
            MPI_Bcast(&ccid[0], ccid_length, MPI_CHAR, 0, MPI_COMM_WORLD);
            // cout << "节点 " << world_rank << " 到达i3同步墙" << endl;
            MPI_Barrier(MPI_COMM_WORLD);
            // cout << "节点 " << world_rank << " 离开i3同步墙" << endl;
        }
        if (i == 6) {
            ...
            MPI_Bcast(&ccid_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            // cout << "ccid_length: " << ccid_length << endl;
            if (world_rank != 0) {
                // cout << "这不是0号节点" << endl;
                // 调整字符串大小以适应接收的数据
                ccid.resize(ccid_length);
            ...
            if (world_rank != 0) {
                string new_query = str1 + ccid + str2;
