...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-8-random.txt";
    string neo4jResult;
    string cid = " ";
    string ccid = " ";
    string pid = " ";
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[2327680];
                    MPI_Recv(buffer, 2327680, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
        ...
        if (i == 4) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[3276800];
                    MPI_Recv(buffer, 3276800, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
