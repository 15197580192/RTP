...
int main(int argc, char** argv) {
    ...
    string file_path = "./queries/interactive-complex-1-other-new.txt";
    string neo4jResult;
    string tid = " ";
    string tid1 = " ";
    string tid2 = " ";
    ...
    vector<Query3Result> query4Results;
    vector<Query3Result> query5Results;
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (i == 1) {
            ...
            if (world_rank != 0) {
                ...
                for (int j = 1; j < world_size; ++j) {
                    char buffer[5200000];
                    MPI_Recv(buffer, 5200000, MPI_CHAR, j, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                    results[j - 1] = string(buffer);
                ...
                tid1 = tid1 + "]\n";
                cout << "q1rs.size(): " << query1Results.size() << endl;

                // ofstream foutid;
                // foutid.open("know*1_2_id_list.txt");
                ...
                //     foutid << query1Results[j].friendPerson.id << " " << query1Results[j].friendPerson.firstName << " " << query1Results[j].friendPerson.lastName << " " << query1Results[j].friendPerson.birthday << " " << query1Results[j].friendPerson.creationDate << endl;
                // }
        ...
        if (i == 2) {
            ...
            for (int t = 0; t < epoch; t++) {
                ...
                for (int a = t * 100; a < min((t + 1) * 100, int(tlist.size())); a++) {
                    if (a % 100 == 0) {
                ...
                if (world_rank != 0) {
                    ...
                    for (int j = 0; j < query3_ans_list.size(); j++) {
                        ...
                        for (int k = 0; k < query3Results.size(); k++) {
                            if (query3Results[k].friendPerson.id == q3.friendPerson.id) {
                                flag = true;
                                break;
                            }
                        }
            ...
            if (world_rank == 0) {
                ...
                for (int j = 0; j < query3Results.size(); j++) {
                    if (j == 0) {
                        tid2 = tid2 + query3Results[j].friendPerson.id;
                    }
                    else {
                        tid2 = tid2 + ", " + query3Results[j].friendPerson.id;
                    }
        ...
        if (i == 3) {
            ...
            MPI_Bcast(&tid2_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
            // cout << "tid2_length: " << tid2_length << endl;
            if (world_rank != 0) {
                // cout << "这不是0号节点" << endl;
                tid2.resize(tid2_length);
            }
            ...
            for (int t = 0; t < epoch; t++) {
                ...
                if (world_rank != 0) {
                    ...
                    for (int j = 0; j < query4_ans_list.size(); j++) {
                        ...
                        bool flag = false;
                        for (int k = 0; k < query1Results.size(); k++) {
                            if (query1Results[k].friendPerson.id == q3.friendPerson.id) {
                                flag = true;
                                break;
                            }
                        }
        ...
        //     MPI_Bcast(&tid3_length, 1, MPI_INT, 0, MPI_COMM_WORLD);
        //     // cout << "tid3_length: " << tid3_length << endl;
        //     if (world_rank != 0) {
        //         // cout << "这不是0号节点" << endl;
        //         tid3.resize(tid3_length);
        //     }
        //     MPI_Bcast(&tid3[0], tid3_length, MPI_CHAR, 0, MPI_COMM_WORLD);

        //     vector<string> tlist;
        //     pos = tid3.find("[");
        //     string ttid = tid3.substr(pos + 1);
        //     ttid = ttid.substr(0, ttid.length() - 2);
        //     // cout << "ttid: " << ttid << endl;
        //     stringSplit(ttid, ", ", tlist);
        //     // cout << tlist[0] << endl;
        ...
        //                 Query3Result q3;
        //                 q3 = parseQuery3Results(query5_ans_list[j]);
        //                 q3.distance = 5;
        //                 bool flag = false;
        //                 for (int k = 0; k < query1Results.size(); k++) {
        //                     if (query1Results[k].friendPerson.id == q3.friendPerson.id) {
