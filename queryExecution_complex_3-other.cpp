...
int main() {
    ...
    string file_path = "./queries/interactive-complex-3-other.txt";
    // string file_path = "./queries/try_query-1.txt";
    string nowIP = "bolt://139.9.250.196:7687/";
    // getIPList(ip_path);
    string neo4jResult, cid, cities_id, tid, tid1;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 2) {
            ...
            for (int j = 0; j < query3Results.size(); j++) {
                if (j == 0) {
                    tid = tid + query3Results[j].friendPerson.id;
                }
                else {
                    tid = tid + ", " + query3Results[j].friendPerson.id;
                }
        ...
        if (i == 3) {
            ...
            neo4jResult = connect2Neo4j(new_query, nowIP);
            // cout << "neo4jResult: " << neo4jResult << endl;
            query3Other_ans_list = getAnsList(neo4jResult);
            ...
            for (int j = 0; j < query3Results.size(); j++) {
                if (j == 0) {
                    tid1 = tid1 + query3Results[j].friendPerson.id;
                }
                else {
                    tid1 = tid1 + ", " + query3Results[j].friendPerson.id;
                }
        ...
        if (i == 4) {
            ...
            string new_query = str1 + tid1 + "AND NOT city IN [" + cities_id + "]\n" + str2;
            // cout << "new_query:" << new_query << endl;
            neo4jResult = connect2Neo4j(new_query, nowIP);
            // cout << "neo4jResult: " << neo4jResult << endl;
            ...
            for (int j = 0; j < query7_ans_list.size(); j++) {
                ...
                for (int k = 0; k < q7.message_list.size(); k++) {
                    for (int a = 0; a < query5Results.size(); a++) {
                        ...
                        if (q7.message_list[k] == query5Results[a].message.id) {
                            q7.xCount += query5Results[a].messageX;
                            q7.yCount += query5Results[a].messageY;
                            break;
                        }
                    }
                }
                q7.xyCount = q7.xCount + q7.yCount;
                // cout << "xCount: " << q7.xCount << endl;
                // cout << "yCount: " << q7.yCount << endl;
                // cout << "xyCount: " << q7.xyCount << endl;
            ...
            for (int j = 0; j < query7_ans_list.size(); j++) {
                ...
                for (int k = 0; k < q7.message_list.size(); k++) {
                    for (int a = 0; a < query6Results.size(); a++) {
                        // cout << "query5Results[a].message.id:" << query5Results[a].message.id << endl;
                        if (q7.message_list[k] == query6Results[a].message.id) {
                            q7.xCount += query6Results[a].messageX;
                            q7.yCount += query6Results[a].messageY;
                            break;
                        }
                    }
                }
