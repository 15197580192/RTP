...
int main() {
    ...
    // getIPList(ip_path);
    string neo4jResult, tid, tid1, fid, pid;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            for (int j = 0; j < query1Results.size(); j++) {
                if (j == 0) {
                    tid1 = tid1 + query1Results[j].otherPerson.id;
                }
                else {
                    tid1 = tid1 + ", " + query1Results[j].otherPerson.id;
                }
