...
struct Query2Result {
    ...
    string commentOrPostId;
    string commentOrPostContent;
    string commentOrPostCreationDate;
    int64_t copCreationDate;
};
...
Query2Result parseQuery2Results(string data) {
    ...
    for (int i = 0; i < sub_data.size(); i++) {
        ...
        string tstr = sub_data[i].substr(pos + 2);
        if (i == 0) {
            q2.personId = tstr;
        }
        if (i == 1) {
            q2.personFirstName = tstr;
        }
        if (i == 2) {
            q2.personLastName = tstr;
        }
        if (i == 3) {
            q2.commentOrPostId = tstr;
        }
        if (i == 4) {
            q2.commentOrPostContent = tstr;
        }
        if (i == 5) {
            q2.commentOrPostCreationDate = tstr;
            string ttime = tstr.substr(1);
            ttime = ttime.substr(0, ttime.length() - 1);
            // cout << "ttime: " << ttime << endl;
            int64_t timestamp = convertToTimestamp(ttime);
            q2.copCreationDate = timestamp;
        }
    }
    return q2;
}
...
bool cmp(Query2Result a, Query2Result b) {
    if (a.copCreationDate != b.copCreationDate) {
        return a.copCreationDate > b.copCreationDate;
    }
    if (a.commentOrPostId.length() != b.commentOrPostId.length()) {
        return a.commentOrPostId.length() < b.commentOrPostId.length();
    }
    return a.commentOrPostId < b.commentOrPostId;
}

void print(Query2Result q2) {
    cout << "---------------------------------------" << endl;
    cout << "'personId': " << q2.personId << ", 'personFirstName': " << q2.personFirstName << ", 'personLastName': " << q2.personLastName;
    cout << ", 'commentOrPostId': " << q2.commentOrPostId << ", 'commentOrPostContent': " << q2.commentOrPostContent;
    cout << ", 'commentOrPostCreationDate': " << q2.commentOrPostCreationDate << endl;
}

int main() {
    auto start = std::chrono::high_resolution_clock::now();

    string file_path = "./queries/interactive-complex-9-other.txt";
    // string file_path = "./queries/try_query-1.txt";
    string nowIP = "bolt://139.9.250.196:7687/";
    // getIPList(ip_path);
    string neo4jResult, tid, tid1;
    ...
    for (int i = 0; i < query_list.size(); i++) {
        ...
        if (i == 1) {
            ...
            neo4jResult = connect2Neo4j(query_list[i], nowIP);
            cout << "neo4jResult:" << neo4jResult << endl;
            query1Other_ans_list = getAnsList(neo4jResult);
            ...
            for (int j = 0; j < query1Other_ans_list.size(); j++) {
                ...
                if (q1.friendPerson.id == "87095182") {
                    break;
                }
                bool flag = false;
                for (int k = 0; k < query1Results.size(); k++) {
                    if (query1Results[k].friendPerson.id == q1.friendPerson.id) {
                        flag = true;
                        break;
                    }
                }
        ...
        if (i == 2) {
            ...
            for (int j = 0; j < query2_ans_list.size(); j++) {
                ...
                // cout << "q2.copCreationDate: " << q2.copCreationDate << endl;
                if (q2.copCreationDate < 1347499953842) {
